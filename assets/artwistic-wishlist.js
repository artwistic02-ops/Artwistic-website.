 ...
 *
 * Entries older than 30 days are dropped on read (real, enforced expiry,
 * not just copy) — see EXPIRY_MS below.
 */
(function () {
  'use strict';
  ...
  function pruneExpired() {
    ...
    var now = Date.now();
    var expired = entries.filter(function (entry) {
  ...
  function moveToBag(button) {
    ...
    if (button.dataset.multiVariant === 'true') {
      /* Multi-variant: reuse the shared quick-add attributes so the site's
         real size-sheet (assets/artwistic-product-card.js) opens instead
         of guessing a variant. */
